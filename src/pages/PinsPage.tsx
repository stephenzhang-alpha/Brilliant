...
/**
 * Page 4 — Pull the Pins, the finale. A self-contained canvas game embedded via
 * <iframe> that talks back over postMessage (`atb:victory {power}` once, and
 * `atb:reset` on a fresh playthrough). This is the terminal page: clearing it
 * banks the score and shows a "you finished Algebra Quest" celebration. There
 * is no further page to gate.
 */
      ...
      <div className="w-full" style={{ maxWidth: 1100 }}>
        <div className="text-center text-white">
          ...
          <p className="mx-auto mt-2 max-w-xl text-sm text-white/85">
            ...
            hero and trap the hazards 🔥 in the drains. Clear all three levels to complete your quest!
          </p>
        </div>
          ...
          {result !== null && (
            <div className="absolute inset-0 flex items-center justify-center px-4 animate-fadein">
              <div className="bg-white rounded-3xl p-7 text-center shadow-2xl max-w-sm">
                ...
                <p className="font-display font-extrabold text-2xl mt-1">Algebra Quest complete!</p>
                <p className="text-text-muted text-sm mt-2">
                  You banked{' '}
                  <span className="font-bold text-success">+{result.toLocaleString()}</span> from the
                  finale. Your total is now{' '}
                  <span className="font-bold text-primary">{overall.toLocaleString()}</span> — you
                  played through every stage of the quest. 🎉
                </p>
                <div className="mt-5 flex flex-col gap-2">
                  <Link
                    to="/leaderboard"
                    className="btn-pop bg-primary text-white font-display font-bold px-7 py-3 rounded-xl"
                  >
                    🏆 See your rank
                  </Link>
                  <button
